...
// Small branches (< 1k lines): 1000 tokens
// Medium branches (1k-10k lines): 2000 tokens (default)
// Large branches (10k-50k lines): 3000-4000 tokens
// Massive branches (50k+ lines): 5000 tokens
...
export default function (pi: ExtensionAPI) {
	...
	async function generateContext(
		...
		const task = `OUTPUT ONLY THE MARKDOWN SUMMARY BELOW. NO CONVERSATIONAL TEXT.
...
OUTPUT FORMAT (start immediately with ## Purpose, no preamble):
...
`.trim();
		...
		return new Promise<string>((resolve) => {
			...
			child.on("error", (error) => {
				resolve(`(Failed to spawn pi: ${error.message})`);
			});
			...
			child.on("close", (code) => {
				if (code === 0 && stdout.trim()) {
					...
					// Strip conversational preamble - find first ## heading
					const headingMatch = output.match(/^##\s/m);
					if (headingMatch && headingMatch.index !== undefined && headingMatch.index > 0) {
						// Found a heading after some preamble - strip everything before it
						output = output.substring(headingMatch.index);
					}
					
					resolve(output);
				} else {
					const errorMsg = stderr || `Process exited with code ${code}`;
					resolve(`(Failed to generate context: ${errorMsg})`);
				}
			});
			...
			setTimeout(() => {
				if (!child.killed) {
					child.kill();
					resolve("(Context generation timed out after 60 seconds)");
				}
			}, 60000);
		});
	}
	...
	/**
	 * Write branch context to file that pi auto-loads
	 */
	async function writeBranchContextFile(
		branch: string,
		baseBranch: string,
		context: string,
	): Promise<void> {
		const piAgentDir = path.join(os.homedir(), ".pi", "agent");
		const contextFile = path.join(piAgentDir, "BRANCH_CONTEXT.md");

		const content = `# Feature Branch Context
...
*Auto-generated by branch-context extension. Run \`/refresh-branch-context\` to update.*
`;
	 ...
	 */
	function removeBranchContextFile(): void {
		const piAgentDir = path.join(os.homedir(), ".pi", "agent");
		const contextFile = path.join(piAgentDir, "BRANCH_CONTEXT.md");
	...
	/**
	 * Hook: Inject branch context into system prompt
	 */
	pi.on("before_agent_start", async (event, ctx) => {
		const branch = await getCurrentBranch();
		if (!branch) return; // Not in git repo
		...
		const cache = loadCache(branch);
		if (!cache) return; // No cache - user hasn't enabled context
		...
		// Cache valid - inject into system prompt
		const branchContext = `
...
`.trim();
	...
	pi.registerCommand("branch-context", {
		...
		handler: async (_args, ctx) => {
			...
			if (!cache) {
				ctx.ui.notify(
					`No context for ${branch}. Run /refresh-branch-context to enable.`,
					"info",
				);
				return;
			}
			...
			const cachePath = path.join(
				CACHE_DIR,
				`${cache.branch.replace(/[^a-zA-Z0-9-_]/g, "_")}.json`,
			);
			
			ctx.ui.notify(
				`✓ Context active for ${branch} (vs ${baseBranch})\nCached: ${timestamp}\nFile: ${cachePath}`,
				"success",
			);
			
			// Return full context for display
